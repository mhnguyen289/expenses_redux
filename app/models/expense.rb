class Expense < ApplicationRecord
  ...
  def self.expenses_and_lent(user_id)
    connection.select_all(self.sql_expenses_and_lent(user_id))
  end

  def self.expenses_and_borrowed(user_id)
    connection.select_all(self.sql_expenses_and_borrowed(user_id))
  end

  def self.expenses_and_debts_between(user_id, friend_id)
